<?php 
    ...
    //Include all blocks - .acf.php
        add_action('acf/init', 'braftonium_start_legacy_blocks');
        function braftonium_start_legacy_blocks(){
    ...
    //Create category for blocks
        add_filter( 'block_categories_all' , function( $categories ) {
            foreach ( $categories as $category ) {
                if ( isset( $category['slug'] ) && 'braftonium-legacy' === $category['slug'] ) {
                    return $categories;
                }
            }

            $categories[] = array(
                'slug'  => 'braftonium-legacy',
                'title' => 'Braftonium - Legacy'
            );
            return $categories;
        } );
