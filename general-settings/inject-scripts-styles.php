...
function braftonium_save_injector() {
    ...
    braftonium_legacy_acf_update_option( 'braftonium_injector', $rules );
...
function braftonium_render_injector_page() {
    ...
    $rules = braftonium_get_global_injector_option();
    ?>
    <div class="wrap">
        ...
        <form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
            ...
            <p class="description"><?php esc_html_e( 'For inline rules, enter the raw JS or CSS only — do not include <script> or <style> tags. Braftonium adds them automatically.', 'braftonium' ); ?></p>
...
function braftonium_render_injector_metabox( $post ) {
    ...
    $rules = braftonium_get_local_injector_option( $post->ID );
    if ( ! is_array( $rules ) ) {
        $rules = array();
    }

    echo '<p>' . esc_html__( 'These rules only apply to this post/page.', 'braftonium' ) . '</p>';
    echo '<p class="description">' . esc_html__( 'For inline rules, enter the raw JS or CSS only — do not include <script> or <style> tags. Braftonium adds them automatically.', 'braftonium' ) . '</p>';
...
function braftonium_save_local_injector( $post_id ) {
    ...
    if ( function_exists( 'update_field' ) ) {
        update_field( 'braftonium_injector', $rules, $post_id );
    }
}
...
function injectionsList() {
    $injections = braftonium_get_global_injector_option();
    ...
    if ( is_singular() ) {
        ...
        if ( $post_id ) {
            $local_injections = braftonium_get_local_injector_option( $post_id );
